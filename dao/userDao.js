...
let getCredentials = function(req) {
  const credPromise = new Promise((reslv, rejct) => {
    ...
    connection.query(sql, [req.body.email, password], (err, rows, fields) => {
      ...
      if (rows) {
        rows.forEach(user => {
          let currentUser = { 'id': user.user_id, 'username': user.username, 'email': user.email}
...
let getUser = function(id) {
  const userPromise = new Promise((resolve, reject) => {
    const connection = defaultDao.getDatabaseConnection();
    connection.connect();
    let sql = "SELECT uid, uname, birthdate, gender, email from people natural join credential where uid = ?";
...
let createPerson = function(req) {
  return new Promise((resolve, reject) => {
    ...
    connection.query(sql1, [req.body.username, req.body.birthdate, req.body.gender], 
      (err, result) => {
        if (err) {
        ...
        if (result) {
          ...
          credPromise.then((credentialUserId) => {
...
let createCredential = function(connection, req, userId) {
  return new Promise((resolve, reject) => {
    ...
    connection.query(sql2, [userId, req.body.email, req.body.password, req.body.securityQuestion, req.body.securityAnswer], 
      (err, result) => {
        if (err) {
